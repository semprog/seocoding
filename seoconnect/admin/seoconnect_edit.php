...
if($ID > 0)
{
	...
	$seoconnect_result = $SeoLink->GetSeoLinkByID($ID);
	$seoconnect_result->ExtractFields("str_");
	$seoconnect_iblocks = $SeoLink->GetIBlocksByID($ID);
	$SeoIBlocks = array();
	while ($item = $seoconnect_iblocks->GetNext()) {
		$SeoIBlocks[] = $item["BID"];
	} 
}
else {
...
if(strlen($Update) > 0 && check_bitrix_sessid())
{
	...
	$SeoLink = new CSeoLink;
	$res = $SeoLink->Update($ID, $arFields);
	...
	else
	{
		...
		if(strlen($save)>0)
		{
			LocalRedirect("/bitrix/admin/seoconnect_list.php?lang=".$lang);
		}
		else {
			LocalRedirect("/bitrix/admin/seoconnect_edit.php?lang=".$lang."&ID=".urlencode($ID));
		}
	}
...
<?php 
		...
		$strSql = "SELECT `ID`,`NAME` FROM `b_iblock` WHERE `ACTIVE` = 'Y' ORDER BY `NAME` ASC";
...
<tr>
	...
	<tr>
		...
		<td>
			...
			<?foreach($arIBlocks as $item):?>
					<OPTION VALUE="<?=$item["ID"];?>" <?if(in_array($item["ID"],$SeoIBlocks)):?>selected<?endif;?>><?=$item["NAME"];?></OPTION>
				<?endforeach;?>
			</select>
		</td>		
	...
	if($ID>0)
	{
		$WRD =CSeoLinkWords::GetList($ID);
		$k = 0;
		while ($WR = $WRD->GetNext())
		{
			
			echo '<tr><td>'.GetMessage("SEOLINK_WORDS").' <input name="NAME['.$k.']" value="'.$WR['NAME'].'" type="text" size="18">';
			echo ' <span title="'.GetMessage("SEOLINK_LINKS").'"> '.GetMessage("SEOLINK_LINKS").' <input name="LINK['.$k.']" value="'.$WR['LINK'].'" size="18" type="text" id="LINK['.$k.']"></span>';
			echo ' <span title="'.GetMessage("SEOLINK_ACTIVITY").'"> Акт. <input name="ACTIVE['.$k.']" value="Y" '.($WR['ACTIVE']!="N"?"checked":"").' type="checkbox"/></span>';
			echo '</td></tr>';
			$k++;
